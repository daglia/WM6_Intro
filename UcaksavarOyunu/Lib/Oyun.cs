...
namespace UcaksavarOyunu.Lib
{
    public class Oyun
    {
        ...
        private void Tmr_Ucak_Tick(object sender, System.EventArgs e)
        {
            foreach (IHareketEdebilir hareket in Ucaklar)
            {
                hareket.HareketEt(Yonler.Asagi);
            }
        }

        private void Tmr_Kontrol_Tick(object sender, System.EventArgs e)
        {
            foreach (Ucak ucak in Ucaklar)
            {
                Rectangle ru = new Rectangle(); //Uçak
                Rectangle rr = new Rectangle(); //Roket
